...
export const App = () => {
  ...
  const [year, setYear] = useState<string | undefined>("2023"); //リリース情報をStateに設定
  const [season, setSeason] = useState<string | undefined>("spring"); //リリース情報をStateに設定
  ...
  const getData = async (year?: string) => {
    ...
    //fetch()で非同期通信を使ってリクエストとレスポンス取得を行う
    //パラメータ参照（https://developers.annict.com/docs/rest-api/v1/works）
    const res = await fetch(
      `${endpoint}/?filter_season=${year}-${season}&${sort}&access_token=${access_token}`
    );
    const data = await res.json(); //json形式にする
    return data;

    //テスト用URL// 'https://jsonplaceholder.typicode.com/posts');
  };
